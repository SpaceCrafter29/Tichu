...
// Feste uids mit Admin-Rechten (jeden Sitz kicken, jeden Tisch löschen - siehe
// firestore.rules: isAdmin()). Rein clientseitig nur fürs Ein-/Ausblenden der Admin-UI -
// die eigentliche Durchsetzung passiert in den Firestore-Regeln, ein Eintrag hier allein
// gibt noch keine Rechte.
export const ADMIN_UIDS = ['HNaYIyZ44qgLwWF12XRyA6jQRjM2'];
...
// Vorgegebene Auswahl fürs Tisch-Erstellen (siehe lib/room.js) - entweder eine feste Rundenzahl
// ("Best of N", Sieger nach N Runden hat am Ende mehr Punkte) oder eine Zielpunktzahl (Match
// endet, sobald ein Team dieses Ziel erreicht/überschreitet). Kein freies Zahlenfeld mehr, um
// Tische vergleichbar/übersichtlich zu halten.
export const ROUND_PRESETS = [1, 2, 3];
export const POINT_PRESETS = [200, 500, 1000];
export const DEFAULT_WIN_CONDITION = { type: 'points', target: 1000 };

// Liest die winCondition eines Tisches robust aus - inkl. Fallback für alte Tische von vor
// diesem Feature (die hatten stattdessen ein einfaches `target`-Zahlenfeld).
export function getWinCondition(game) {
  if (game && game.winCondition) return game.winCondition;
  if (game && typeof game.target === 'number') return { type: 'points', target: game.target };
  return DEFAULT_WIN_CONDITION;
}

export function describeWinCondition(game) {
  const wc = getWinCondition(game);
  return wc.type === 'rounds' ? `Best of ${wc.count}` : `Ziel ${wc.target}`;
}

function normalizeWinCondition(winCondition) {
  if (winCondition && winCondition.type === 'rounds' && ROUND_PRESETS.includes(winCondition.count)) {
    return { type: 'rounds', count: winCondition.count };
  }
  if (winCondition && winCondition.type === 'points' && POINT_PRESETS.includes(winCondition.target)) {
    return { type: 'points', target: winCondition.target };
  }
  return DEFAULT_WIN_CONDITION;
}
...
// spielen") - siehe board.html. Ergibt nur bei winCondition.type 'points' einen Sinn.
export async function createTable(mode, { name = randomTableName(), winCondition = DEFAULT_WIN_CONDITION, catchUpRule = true } = {}) {
  const normalized = normalizeWinCondition(winCondition);
  const ref = await addDoc(collection(db, 'games'), {
    ...
    name,
    mode,
    catchUpRule: normalized.type === 'points' ? !!catchUpRule : false,
  });
  return ref.id;
}
...
export async function joinTable(gameId, uid) {
  ...
  return runTransaction(db, async (tx) => {
    ...
    tx.set(mySeatRef, {
      claimedByUid: uid, anonToken, joinedAt: serverTimestamp(), hand: myHand, restHand: myRest,
      lastActive: serverTimestamp(), // siehe heartbeat() - solange der Tisch noch wartet, erkennt
      // das andere Mitspieler an diesem Tisch daran, ob wer eingeschlafen ist (Inaktivitäts-Kick).
    });
...
// Löscht das komplette events-Log eines Tisches (bestmöglich, ein Fehlschlag hier soll das
// eigentliche Verlassen/Kicken nicht scheitern lassen). Nötig, weil derselbe gameId nach einem
// Vollreset wiederverwendet wird (nächste vier Beitretenden landen am selben Tisch) - ohne das
// blieben Ereignisse der ALTEN Partie liegen und würden beim nächsten replay() (mit dem frischen
// Deal, aber demselben Log) mitverarbeitet. Erlaubt laut firestore.rules nur für einen bereits
// komplett leeren, wartenden Tisch (siehe isTableFullyEmpty()) - kann also nie eine laufende
// Partie treffen.
async function clearEvents(gameId) {
  try {
    const snap = await getDocs(collection(db, 'games', gameId, 'events'));
    await Promise.all(snap.docs.map((d) => deleteDoc(d.ref)));
  } catch (e) {
    console.warn('events-Log konnte nicht (komplett) geräumt werden:', e);
  }
}
...
export async function leaveTable(gameId, seatIndex) {
  ...
  if (didFullReset) await clearEvents(gameId);
}
...
// Alle vier privaten Sitz-Dokumente eines Tisches auf einmal beobachten (nur möglich, sobald man
// selbst an diesem Tisch sitzt, siehe firestore.rules) - fürs Erkennen inaktiver Mitspieler in
// der Lobby (siehe heartbeat()/LOBBY_STALE_MS in room.js) und fürs Admin-Dashboard.
export function watchAllSeats(gameId, callback) {
  return onSnapshot(collection(db, 'games', gameId, 'private'), (snap) => {
    const bySeat = {};
    for (const d of snap.docs) bySeat[d.id] = d.data();
    callback(bySeat);
  });
}

// Während man noch in einem wartenden Tisch sitzt, alle paar Sekunden aufrufen (siehe room.js).
// Lässt andere Mitspieler an diesem Tisch erkennen, dass man noch da ist (siehe watchAllSeats()) -
// wer zu lange nicht mehr "geheartbeated" hat, gilt als inaktiv und darf laut firestore.rules von
// einem Mitspieler aus dem Tisch entfernt werden (derselbe leaveTable()-Aufruf wie beim eigenen
// Verlassen, nur mit fremdem seatIndex).
export async function heartbeat(gameId, seatIndex) {
  await updateDoc(doc(db, 'games', gameId, 'private', SEAT_DOC_IDS[seatIndex]), { lastActive: serverTimestamp() });
}
...
// Für das Admin-Dashboard: ALLE Tische, unabhängig von Status/Modus (im Gegensatz zu
// watchOpenGames(), das nur wartende Tische eines Modus zeigt).
...
// Admin-Aufräumen: löscht einen Tisch samt aller privaten Sitz-Dokumente, unabhängig vom Status
// oder davon, ob noch Sitze belegt sind (im Gegensatz zu deleteEmptyTable()). Braucht die
// isAdmin()-Ausnahme in firestore.rules.
export async function adminDeleteTable(gameId) {
  const snaps = await Promise.all(SEAT_DOC_IDS.map((id) => getDoc(doc(db, 'games', gameId, 'private', id))));
  await Promise.all(snaps.filter((s) => s.exists()).map((s) => deleteDoc(s.ref)));
  await clearEvents(gameId);
  await deleteDoc(doc(db, 'games', gameId));
}
