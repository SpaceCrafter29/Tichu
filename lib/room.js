...
import {
  ...
  SEAT_DOC_IDS, randomTableName, ROUND_PRESETS, POINT_PRESETS, DEFAULT_WIN_CONDITION,
  describeWinCondition, heartbeat, watchAllSeats,
} from './lobby.js';
...
export async function mountRoomPage(mode) {
  ...
  let creating = false;
  let pendingName = '';
  let pendingWinCondition = DEFAULT_WIN_CONDITION;
  ...
  function renderCreatePanel() {
    ...
    wcLabel.style.marginTop = '10px';
    wcLabel.textContent = 'Wie wird gewonnen?';
    createPanel.appendChild(wcLabel);
    ...
    function sameWinCondition(a, b) {
      return a.type === b.type && (a.type === 'rounds' ? a.count === b.count : a.target === b.target);
    }
    function addPresetBtn(wc, label) {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = sameWinCondition(wc, pendingWinCondition) ? '' : 'secondary';
      btn.style.padding = '6px 12px';
      btn.style.fontSize = '0.85rem';
      btn.textContent = label;
      btn.addEventListener('click', () => { pendingWinCondition = wc; renderCreatePanel(); });
      presetRow.appendChild(btn);
    }
    for (const n of ROUND_PRESETS) addPresetBtn({ type: 'rounds', count: n }, `Best of ${n} Runde${n > 1 ? 'n' : ''}`);
    for (const p of POINT_PRESETS) addPresetBtn({ type: 'points', target: p }, `${p} Punkte`);
    createPanel.appendChild(presetRow);

    let catchUpCheckbox = null;
    if (pendingWinCondition.type === 'points') {
      ...
      catchUpCheckbox = document.createElement('input');
    ...
    confirmBtn.addEventListener('click', async () => {
      ...
      try {
        const gameId = await createTable(mode, {
          name: pendingName,
          winCondition: pendingWinCondition,
          catchUpRule: catchUpCheckbox ? catchUpCheckbox.checked : false,
        });
  ...
  function renderTableList(games) {
    ...
    for (const game of games) {
      ...
      row.innerHTML = `
        <span>${tableName} <span class="muted">(${4 - openSeats}/4, ${describeWinCondition(game)})</span></span>
      `;
      const btn = document.createElement('button');
  ...
  createTableBtn.addEventListener('click', () => {
    ...
    pendingWinCondition = DEFAULT_WIN_CONDITION;
    renderRoom();
  });
}
